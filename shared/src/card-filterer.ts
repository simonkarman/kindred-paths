...
const check = (
  ...
  return (searchTerm: string) => {
    for (const key of keys) {
      ...
      // If needle starts and ends with quotes, remove them for exact matching
...
const cardFaceTermResolver = (_card: SerializedCard, faceIndex: number, term: string): boolean => {
  ...
  const checks = [
    ...
    check(getFilterKeys('producible-color'), colorNeedle => {
      ...
      if (colorNeedle === 'some' || colorNeedle === 's') {
        return cardFace.producibleColors().length > 0;
      }
      if (colorNeedle === 'multicolor' || colorNeedle === 'multi' || colorNeedle === 'm') {
      ...
      if (colorNeedle === 'colorless' || colorNeedle === 'c') {
        return cardFace.producibleColors().includes('colorless');
      }
      // Convert single character to full color name
      const c = wubrg.includes(colorNeedle as CardColorCharacter)
        ? colorToLong(colorNeedle as CardColorCharacter)
  ...
  // If the search term ends with ":" or '=', it's an incomplete key, so fail the search
  if (searchTerm.endsWith(':') || searchTerm.endsWith('=')) return false;
