...
export async function generateMetadata({ params: _params }: PageProps<{ cid: string; name: string }>): Promise<Metadata> {
  ...
  return {
    title: `Edit ${card?.faces.map(f => f.name).join(' // ') ?? params.cid} - Kindred Paths`,
  }
}

export default async function CardEdit({ params: _params }: Readonly<{ params: Promise<{ cid: string; name: string }> }>) {
  const params = await _params;
  ...
  return (<>
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8">
      ...
      <CardEditor isNewCard={false} initialCard={serializedCard} />
    </div>
  </>);
}
