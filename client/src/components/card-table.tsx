...
export const CardTable = (props: { cards: SerializedCard[] }) => {
  ...
  return <>
    <div className="absolute -mt-9 flex gap-2 items-center justify-end w-300 text-sm mb-2">
      {filters.map(filter => {
        return <button
          key={filter.name}
    ...
    {showPossibleFilters && <div className="bg-zinc-50 p-2 rounded border border-zinc-300 mb-2 w-300">
      <h3 className="font-bold text-sm mb-1">Filters</h3>
      <div className="flex flex-wrap justify-start gap-2">
        {filterCategories.map(category => (<ul
          key={category.category}
          className="flex flex-col gap-1 border rounded border-zinc-200 bg-zinc-100 py-1 px-3"
        >
    ...
    <ul className='flex flex-col items-start mb-5'>
      ...
      {cards.map((card) => {
        ...
        >
          <span className="inline-block w-18 space-x-1">
            <Link
              className="text-zinc-500 hover:text-orange-700 active:text-orange-500"
              href={`/card/${card.id}`}
            ...
            <button
              className="text-zinc-500 hover:text-red-700 active:text-red-500"
              onClick={() => del(card.id)}
            ><FontAwesomeIcon icon={faTrashCan} /></button>
          </span>
          <span className="inline-block w-8 text-sm text-zinc-800">{card.collectorNumber}</span>
          ...
          <span className="inline-block w-40 pl-2 text-zinc-500 text-xs tracking-wide overflow-hidden">{tagsAsString(card.tags)}</span>
        </li>
      })}
    </ul>
