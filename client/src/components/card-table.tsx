  ...
  const cardSummaries = props.cardSummaries
    ...
    .sort((a, b) => {
      ...
      if (sortKey.k === 'collector-number') {
        ...
        const colorlessA = a.card.manaCost['colorless'] ?? 0;
        const colorlessB = b.card.manaCost['colorless'] ?? 0;
        if (colorlessA !== colorlessB) {
          return -(colorlessA - colorlessB);
        }
        const aRenderedCost = new Card(a.card).renderManaCost();
        const bRenderedCost = new Card(b.card).renderManaCost();
        return aRenderedCost.localeCompare(bRenderedCost);
        ...
        const typeLineA = [...a.card.types, '-', a.card.supertype ?? ' ', '-', ...(a.card.subtypes ?? [])]
        const typeLineB = [...b.card.types, '-', b.card.supertype ?? ' ', '-', ...(b.card.subtypes ?? [])];
    ...
    <ul className='flex flex-col items-start'>
      ...
      {cardSummaries.map((summary) => {
        ...
        >
          ...
          <span className="inline-block w-12 text-center">{summary.hasArt ? 'yes' : 'no'}</span>
        </li>
      })}
    </ul>
  </>;
