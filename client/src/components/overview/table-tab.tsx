    ...
    <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          {cards.flatMap((card) => card.faces).map(face => {
            const faceIndex = face.card.faces.indexOf(face);
            ...
            const defaultPaddingY = `border-y py-2 ${borderColor}`;
            const paddingY = face.card.faces.length === 1
              ? defaultPaddingY
              : (isFirstFaceOfDualCard ? `border-t pt-4 pb-0.5 ${borderColor}` : `pt-0 pb-4 border-b ${borderColor}`);
            const doubleRowPaddingY = defaultPaddingY;

            const cardDisplayName = face.name + (isFirstFaceOfDualCard ? ' //' : '');
            const creatableTokenNames = face.getCreatableTokenNames();

            return <tr
              key={`${face.card.id}-${faceIndex}`}
